...
export default function ManageQRs() {
  ...
  const fetchUserDetails = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/getUserDetails?userObjId=${user.objId}`
      );
  ...
  const handleCreateQR = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/qr/create`,
        {
          creator: {
        ...
        {!qrListLoading &&
          (filteredQrs.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              {filteredQrs
                .filter((qr) => qr.type === "business_card")
                .map((qr) => (
                  <div
                    key={qr.qrId}
                  ...
                  >
                    ...
                    <div className="hidden sm:flex sm:col-span-12 md:col-span-3 justify-end gap-2 mt-2 sm:mt-0">
                      <button
                        onClick={() =>
                          downloadImage(qr.imageUrl, `qr-${qr._id}.png`)
                        }
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-[1px] z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-8 w-full max-w-sm relative shadow-lg animate-fade-in">
            ...
            <div className="flex items-center justify-between gap-3">
              <button
                onClick={() =>
                  downloadImage(selectedQR.imageUrl, `qr-${selectedQR._id}.png`)
                }
              ...
              {userData.credits >= 1 ? (
                ...
                <div className="mt-2 text-sm text-red-600">
                  <p className="font-medium">
                    You don't have enough credits to create a QR.
                  </p>
